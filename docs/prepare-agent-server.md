...
- detects whether the installed OpenClaw plugin CLI supports capability consent and passes `--accept-capabilities` only on those releases; local relay-channel archives also use explicit `--force`, keeping noninteractive snapshot preparation compatible with both OpenClaw 2026.7.1 and 2026.8.1+ trust rules;
