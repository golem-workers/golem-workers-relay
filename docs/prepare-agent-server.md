...
- installs `pnpm`, installs the latest OpenClaw through the pnpm global package tree, prepares runtime dependencies (`memory-lancedb-pro@beta`, `grammy`), preinstalls the `relay-channel` plugin into `/root/.openclaw/workspace/plugins/relay-channel`, and installs full `playwright`;
