...
- detects whether the installed OpenClaw plugin CLI supports capability consent and passes `--accept-capabilities` only on those releases; local relay-channel archives also use explicit `--force`; database-first releases keep install metadata in OpenClaw's plugin index instead of the retired authored `plugins.installs` config, while legacy releases retain their existing config records;
