...
- installs `pnpm`, installs the latest OpenClaw through a hoisted pnpm global package tree, prepares runtime dependencies (`memory-lancedb-pro@beta`, `grammy`, `@buape/carbon` for the current OpenClaw Discord UI import bug), preinstalls the `relay-channel` plugin into `/root/.openclaw/workspace/plugins/relay-channel`, and installs full `playwright`;
