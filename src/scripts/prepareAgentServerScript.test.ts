...
const prepareAgentServerScriptPath = resolve(
  process.cwd(),
...
describe("prepare-agent-server snapshot preparation", () => {
  ...
  it("accepts plugin capabilities only when the installed OpenClaw CLI supports it", () => {
    ...
    expect(script).toContain(
      'if [[ "${install_help}" == *"--accept-capabilities"* ]]',
    );
    expect(script).toContain(
  ...
  it("raises the generated gateway unit heap floor before readiness recovery", () => {
    ...
    expect(
      script.indexOf("OPENCLAW_GATEWAY_SNAPSHOT_NODE_OPTIONS"),
    ).toBeLessThan(
  ...
  it("bakes the WhatsApp plugin into provider snapshots before channel warmup", () => {
    ...
    expect(
      script.indexOf('set_step "openclaw_whatsapp_plugin_install"'),
    ).toBeGreaterThan(
  ...
  it("bakes official external provider plugins required by OpenClaw 2026.8.1", () => {
    const script = readFileSync(prepareAgentServerScriptPath, "utf8");

    expect(script).toContain("resolve-openclaw-official-plugin-version.mjs");
    expect(script).toContain('"@openclaw/moonshot-provider"');
    expect(script).toContain('"@openclaw/perplexity-plugin"');
    expect(script).toContain(
    ...
    expect(script).toContain(
      '"telegram", "whatsapp", "moonshot", "perplexity"',
    );
    expect(
      script.indexOf('set_step "openclaw_external_provider_plugins_install"'),
    ).toBeLessThan(
      script.indexOf('set_step "openclaw_snapshot_channels_warmup_start"'),
    );
  });
  ...
  it("bakes curated OpenClaw skills into provider snapshots before onboarding", () => {
    ...
    expect(
      script.indexOf('set_step "openclaw_safe_skills_preinstall"'),
    ).toBeGreaterThan(
      script.indexOf('set_step "openclaw_codex_plugin_install"'),
    );
    expect(
      script.indexOf('set_step "openclaw_safe_skills_preinstall"'),
    ).toBeLessThan(script.indexOf('set_step "openclaw_onboard"'));
  });
});
