...
- installs `pnpm`, installs the latest OpenClaw through a hoisted pnpm global package tree, prepares runtime dependencies (`memory-lancedb-pro@beta`, `grammy`, `@buape/carbon` for the current OpenClaw Discord UI import bug), preinstalls `relay-channel` into the OpenClaw workspace plugin directory, plus full `playwright`;
